@ECHO OFF
:: Process extracts the Survey PnD Print

SET "LogErrFileName=SurveyPndPrint"

ECHO Running batch Survey PnD Print ...

CALL \\TorCwiDev01\BatchFiles\Publication\BatchInclude\MSDosServerTest.bat

SET "StartDateTime=%DateTime%"
SET "ProductType=SurveyPnd"
SET "ProcessName=SurveyPndPrint"
SET "BatchSubDir=survey"
SET "XmlSubDir=pnd"
SET "ErrorFileName=spptmp.err"
SET /a "ErrLevel=0"
...
java -Xms512M -Xmx512M -jar "c:\program files\saxon\saxon.jar" "%OutputPath%\%ProductType%.xml" "%XMLPath%\process\splitfile_cp1252.xsl" 2>> %BatchPath%\%BatchSubDir%\%ErrorFileName%
IF %ERRORLEVEL% NEQ 0 (
  SET /a "ErrLevel+=1"
  SET "Message=##ERROR## %ProductType%: Saxon - there was a problem splitting the file. %Path2BatchMessage%"
  GOTO ExitBatch
) 

"%SedExe%" -f %XMLPath%\process\pdfprint_character.sed %XMLPath%\%XmlSubDir%\%ProductType%.xml > %XMLPath%\%XmlSubDir%\%ProductType%Sed.xml 2>> %BatchPath%\%BatchSubDir%\%ErrorFileName%
IF %ERRORLEVEL% NEQ 0 (
  SET /a "ErrLevel+=1"
...
CALL "c:\program files\xep\xep" -xml %XMLPath%\%XmlSubDir%\%ProductType%Sed.xml -xsl %XMLPath%\%XmlSubDir%\pdf\wrapper_pnd.xsl -ps %XMLPath%\%XmlSubDir%\%ProductType%.ps 2>> %BatchPath%\%BatchSubDir%\%ErrorFileName% 1>NUL
IF %ERRORLEVEL% NEQ 0 (
  SET /a "ErrLevel+=1"
